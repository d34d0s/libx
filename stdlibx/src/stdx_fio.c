#include "../include/stdx_fio.h"
#include "../include/stdx_memory.h"

_stdx_fileio_api* fileio_api = NULL;

u8 _exists_impl(cstr path) {
    FILE* file = fopen(path, "rb");
    if (!file) return STDX_FALSE;
    fclose(file);
    return STDX_TRUE;
}
...
u8 _copy_impl(cstr src, cstr dest, u64 size) {
    ...
    if (!srcf) return STDX_FALSE;   // error: file not found!
    
    FILE* destf = fopen(dest, "wb");
    if (!destf) {
        fclose(srcf);
        return STDX_FALSE;  // error: file not found!
    }
...
u8 _readb_impl(cstr path, u64 size, void* out) {
    FILE* file = fopen(path, "rb");
    if (!file) return STDX_FALSE; // error: file not found!

    u64 read = 0;
    read = fread(out, size, 1, file);
    if (read != size) {
        fclose(file);
        return STDX_FALSE;    // error: failed to fully read data!
    }
...
u8 _remove_impl(cstr path) {
    if (remove(path) != STDX_FALSE) return STDX_FALSE;  // error: failed to delete file!
    return STDX_TRUE;
}

u8 _write_impl(cstr path, cstr lines, u8 preserve) {
    FILE* file = fopen(path, preserve ? "ab" : "wb");
    if (!file) return STDX_FALSE;   // error: failed to write to file!

    u64 size = strlen(lines);
    if (!size) {
        fclose(file);
        return STDX_FALSE;  // error: failed to get size of `lines`!
    }

    u64 written = fwrite(lines, sizeof(char), size, file);
    if (written != size) {
        fclose(file);
        return STDX_FALSE;  // error: failed to write all lines!
    }
...
u8 _process_impl(cstr path, u64 lines, void (*callback)(cstr line)) {
    FILE* file = fopen(path, "rb");
    if (!file) return STDX_FALSE;    // error: failed to open file!

    str buffer = memory_api->alloc(lines, 16);
    if (!buffer) return STDX_FALSE; // error: failed to allocate temporary buffer!
...
u8 stdx_init_fileio(void) {
    fileio_api  = memory_api->alloc(sizeof(_stdx_fileio_api), 16);
    if (!fileio_api) return STDX_FALSE; // error: out of memory!
